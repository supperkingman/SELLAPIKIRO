...
// normalizeClaudeRequestForAgents relaxes Claude Code / Cursor / OpenCode payloads
// for the Grok path (thinking budget quirks, tool-only turns, prefill).
func normalizeClaudeRequestForAgents(req *ClaudeRequest) {
	if req == nil {
		return
	}
	// Drop thinking block for Grok: Grok uses reasoning.effort instead.
	// Strict Anthropic thinking validation would 400 many agent requests.
	req.Thinking = nil
	// Ensure max_tokens is positive for clients that send 0 with tools.
	if req.MaxTokens <= 0 {
		req.MaxTokens = 16384
	}
	// If last message is assistant (prefill), append a minimal user nudge so
	// message chains remain valid for Responses input conversion.
	if len(req.Messages) > 0 {
		last := strings.TrimSpace(req.Messages[len(req.Messages)-1].Role)
		if strings.EqualFold(last, "assistant") {
			req.Messages = append(req.Messages, ClaudeMessage{Role: "user", Content: "Continue."})
		}
	}
}
...
func validateClaudeRequestShape(req *ClaudeRequest) string {
	...
	// Agent clients sometimes send assistant prefill as the last message.
	// Do not hard-fail; translators/Grok path can still proceed. Kiro path
	// will append empty user context below if needed by extractors.
	_ = lastRole
	if !hasUserContext {
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	w.Header().Set("Access-Control-Allow-Headers", "Content-Type, Authorization, X-Api-Key, anthropic-version, anthropic-beta, anthropic-dangerous-direct-browser-access, x-api-key, x-request-id, x-stainless-os, x-stainless-lang, x-stainless-package-version, x-stainless-runtime, x-stainless-runtime-version, x-stainless-arch, x-stainless-retry-count, x-stainless-timeout, openai-organization, openai-project, user-agent, accept, accept-language")
...
func (h *Handler) handleClaudeMessagesInternal(w http.ResponseWriter, r *http.Request) {
	...
	// Agent clients (Claude Code / Cursor / OpenCode / 9router) often send thinking
	// configs, tool-only turns, or assistant-prefill that Kiro validates strictly.
	// Route Grok (explicit + silent) BEFORE strict shape validation so those apps
	// do not get 400s that pure Anthropic would not always raise the same way.
	if IsGrokModel(req.Model) {
	...
	// Anthropic's web_search is a server tool: the upstream generateAssistantResponse
	// endpoint does not execute it, so a request carrying one has to be served
	// through Kiro's MCP relay instead of the normal conversion path.
	//
	// Pure native web_search: relay directly via Kiro MCP.
	if hasWebSearchTool(&req) {
		h.handleWebSearchRequest(w, &req, estimatedInputTokens, apiKeyID)
		return
	}

	// Mixed tools including native web_search: the agentic loop digests web_search
	// internally and passes client tool_use blocks through untouched.
	...
	var handled bool
	if req.Stream {
		handled = h.handleClaudeStream(w, kiroPayload, req.Model, thinking, thinkingResponseOpts, estimatedInputTokens, cacheProfile, apiKeyID)
	} else {
		handled = h.handleClaudeNonStream(w, kiroPayload, req.Model, thinking, thinkingResponseOpts, estimatedInputTokens, cacheProfile, apiKeyID)
	}
	if handled {
		return
	}
...
func (h *Handler) handleClaudeNonStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, thinkingOpts claudeThinkingResponseOptions, estimatedInputTokens int, cacheProfile *promptCacheProfile, apiKeyID string) bool {
	excluded := make(map[string]bool)
	var lastErr error
	reqStart := time.Now()
...
func (h *Handler) handleOpenAIChat(w http.ResponseWriter, r *http.Request) {
	...
	// Grok CLI (Grok Build): route by model id — separate pool, clear errors, no Claude fallback.
	if IsGrokModel(req.Model) {
	...
	var handled bool
	if req.Stream {
		handled = h.handleOpenAIStream(w, kiroPayload, req.Model, thinking, estimatedInputTokens, apiKeyID)
	} else {
		handled = h.handleOpenAINonStream(w, kiroPayload, req.Model, thinking, estimatedInputTokens, apiKeyID)
	}
	if handled {
		return
	}
...
func (h *Handler) handleOpenAIStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, estimatedInputTokens int, apiKeyID string) bool {
	w.Header().Set("Content-Type", "text/event-stream; charset=utf-8")
	w.Header().Set("Cache-Control", "no-cache")
	w.Header().Set("Connection", "keep-alive")
...
func (h *Handler) handleOpenAINonStream(w http.ResponseWriter, payload *KiroPayload, model string, thinking bool, estimatedInputTokens int, apiKeyID string) bool {
	excluded := make(map[string]bool)
	var lastErr error
	reqStart := time.Now()
